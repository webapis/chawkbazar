...
import kategoriler from './../../../public/api/kategoriler.json'
import altKategoriler from './../../../public/api/subcategories.json'
...
export default async function handler(req, res) {
  ...
  for (let query in req.query) {
    if (query === 'q') {
      console.log('q', req.query[query].split('-'))
      const gender = req.query[query].split('-')[0]
      const otherQ = req.query[query].split('-').filter((f, i) => i > 0)
      // console.log('gender',gender)
      // console.log('otherQ',otherQ)
      const katQueries = []
      for (let k of kategoriler.data) {
      ...
      newquery.where.gender = { contains: gender.replace(/ı/g, "i").replace("kiz-cocuk", "_kcocuk").replace("erkek-çocuk", "_ecocuk") }
